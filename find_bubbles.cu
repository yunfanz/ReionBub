 ...
 __global__ void update_sphere_kernel(float* ionized, float* fcollapse, const int w, float R)
{
  ...
  if (fcollapse[p] >= 1/%(ZETA)s) 
  {
  	if (true)
  	{
		...
		int bbx2 = ((i+R+1)>w) ? w : ceil(i+R+1);
		int bby2 = ((j+R+1)>w) ? w : ceil(j+R+1);
		int bbz2 = ((k+R+1)>w) ? w : ceil(k+R+1);
  	...
  	else  //using shared memory, doesn't seem to help much
  	{
		...
		s_I[C_INDEX(tz,ty,tx,BLOCK_SIZE)] = ionized[p];
		__syncthreads();
		int ttx, tty, ttz;
		int bbx1 = ((i-R)<0) ? 0 : floor(i-R);
		int bby1 = ((j-R)<0) ? 0 : floor(j-R);
					...
					if (rsq < R*R) {
						...
						if (ttx<0 || tty<0 || ttz<0 ||
						    ttx>=bdx || tty>=bdy || ttz>=bdz) {
						    	ionized[C_INDEX(kk,jj,ii,w)] = 1.0;
						    }
						else {
							s_I[C_INDEX(ttz,tty,ttx,BLOCK_SIZE)] = 1.0;
						}
					}
				}
			}
		...
		__syncthreads();
		if (s_I[C_INDEX(tz,ty,tx,BLOCK_SIZE)] > 0) { ionized[p] = 1.0; }
